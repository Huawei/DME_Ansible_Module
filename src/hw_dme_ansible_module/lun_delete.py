# -*- coding: utf-8
from hw_dme_ansible_core.volume import delete
...
def run_module(params, ansible_base, module):
    def delete_luns_request_func(param):
        return ansible_base.post(constants.LUN_DELETE_URL, param)

    result = {}
    try:
        result['data'] = delete(params, delete_luns_request_func,
                                ansible_base.task_request,
                                ansible_base.lun_id_convert_func)
        result['msg'] = 'Delete LUNs successfully.'
    except ScriptException as ex:
        result['msg'] = 'Delete LUNs failed, Fault cause: %s.' % ex
        result['failed'] = True
