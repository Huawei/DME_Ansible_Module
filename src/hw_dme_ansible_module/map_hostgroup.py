...
from hw_dme_ansible_core.volume import map_hostgroup

DOCUMENTATION = r'''
---
module: map_hostgroup
description:
  - LUN Mapping host group
Options:
  dme_ip:
    description:
      - DME IP.
...
def run_module(params, ansible_base, module):
    def map_hostgroup_request_func(param):
        return ansible_base.post(constants.MAP_HOSTGROUP_URL, param)

    result = {}

    try:
        result['data'] = map_hostgroup(params,
                                       map_hostgroup_request_func,
                                       ansible_base.task_request,
                                       ansible_base.lun_id_convert_func)
        result['msg'] = 'LUN is successfully mapped to host group.'
    except ScriptException as ex:
        result[
            'msg'] = 'LUN fails to be mapped to host group. Fault cause: %s' % ex
        result['failed'] = True
